...
from old_opacus.opacus import PrivacyEngine

from train_utils import get_device, train, test,sgd_train
...
from change_wide_resnet import WideResNet
...
def main(dataset, augment=False, use_scattering=False, size=None,
         ...
         noise_multiplier=1, max_grad_norm=0.1, epochs=500,
    ...
    # num=0
    # lr_list = [0.1,0.5, 1, 2, 4, 6, 8]
    lr_list = [0.001,0.005,0.01,0.05,0.1,0.5, 1, 2, 4, 6, 8]
    lr = lr_list[num]
    print('lr: ', lr)
    ...
    if input_norm == "BN":
        ...
        model = WideResNet(10, 10, 1, 16, 0, order1=0, order2=0)
    else:
    ...
    for epoch in range(0, epochs):
        ...
        train_loss, train_acc = sgd_train(model, train_loader, optimizer, n_acc_steps=n_acc_steps)
