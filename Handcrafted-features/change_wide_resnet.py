...
class WideResNet(nn.Module):
    def __init__(
        ...
        # self.conv1 = Conv2d(243, nChannels[0], kernel_size=3, stride=1, padding=1)
        self.conv1 = Conv2d(243, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        # self.conv1 = Conv2d(81, nChannels[0], kernel_size=3, stride=1, padding=1)
        # 1st block
    ...
    def forward(self, x):
        # x_resized = F.interpolate(x, size=(72, 72), mode='bilinear', align_corners=False)
        # x_resized = x.reshape(-1,3, 72, 72)
        # print('input size',x_resized.shape)
        # x = x.view(-1, 3, 81 * 8 * 8)  # Flatten the spatial dimensions
        # x = nn.AdaptiveAvgPool2d((72, 72))(x)
        # x = F.interpolate(x, size=(72, 72), mode='bilinear',
        #                   align_corners=False)  # Apply interpolation to change size to [72,72]
        x_resized = x.reshape(-1, 243, 8, 8)
        # print('input size',x_resized.shape)
        out = self.conv1(x_resized)
        # out = self.conv1(x)
        out = self.block1(out)
        out = self.block2(out)
        out = self.block3(out)
        out = self.bn1(self.relu(out)) if self.order2 == 0 else self.relu(self.bn1(out))
        # out = F.avg_pool2d(out, 9)
        out = F.avg_pool2d(out, out.shape[-2:])
        # print('output size of avg_pool',out.shape)
        out = out.view(-1, self.nChannels)
        # print('output size of view',out.shape)
        final = self.fc(out)
        # print('output size of fc',final.shape)
        return final