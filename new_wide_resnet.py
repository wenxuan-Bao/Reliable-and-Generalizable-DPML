...
class BasicBlock(nn.Module):
    def __init__(self, in_planes, out_planes, stride, nb_groups, order,scale):
    ...
    def forward(self, x):
        ...
        if self.scale==False:
            ...
            result=torch.add(skip, out)
            final=self.bn2(result)
            return final
...
class NetworkBlock(nn.Module):
    def __init__(
        self, nb_layers, in_planes, out_planes, block, stride, nb_groups, order,scale
    ):
        super(NetworkBlock, self).__init__()
        self.layer = self._make_layer(
            block, in_planes, out_planes, nb_layers, stride, nb_groups, order,scale
        )

    def _make_layer(
        self, block, in_planes, out_planes, nb_layers, stride, nb_groups, order,scale
    ):
        layers = []
...
class WideResNet(nn.Module):
    def __init__(
        ...
        self.block1 = NetworkBlock(
            n, nChannels[0], nChannels[1], block, 1, nb_groups, order1,scale
        )
        # self.bn1_s = nn.GroupNorm(nb_groups, nChannels[0]) if nb_groups else nn.Identity()
        # 2nd block
        self.block2 = NetworkBlock(
            n, nChannels[1], nChannels[2], block, 2, nb_groups, order1,scale
        )
        # self.bn2_s = nn.GroupNorm(nb_groups, nChannels[1]) if nb_groups else nn.Identity()
        # 3rd block
        self.block3 = NetworkBlock(
            n, nChannels[2], nChannels[3], block, 2, nb_groups, order1,scale
        )
