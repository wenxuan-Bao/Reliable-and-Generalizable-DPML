...
def prepare_data_pathmnist(data_root,batch_size,proportion):
    ...
    # train_loader_at_eval = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=2 * batch_size,
    #                                                    shuffle=False)
    testloader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=batch_size, shuffle=False)

    return (train_dataset, trainloader, testloader)
